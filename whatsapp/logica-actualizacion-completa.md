# Lógica Completa de Actualización de `whatsapp_id` y `whatsapp_number`
...
**Datos disponibles:**
- `socket.user.id`: ID de WhatsApp completo (ej: "138916556447751:lid")
...
1. **`whatsapp_number` no está disponible al conectar:**
   - Solo tenemos `whatsapp_id` desde `socket.user.id`
...
Para actualizar ambos campos automáticamente al conectar, necesitaríamos:

1. **Opción A:** Modificar el frontend para que siempre pase el `phoneNumber` cuando llama a `linkWhatsAppPhone()`
   - Requiere que el usuario ingrese su número manualmente
   - No es completamente automático

2. **Opción B:** Crear un endpoint que devuelva el `whatsapp_number` desde la API de WhatsApp
   - Investigar si Baileys expone el número real en `socket.user`
   - Puede no estar disponible

3. **Opción C:** Mantener el flujo actual (recomendado)
   - Al conectar: Actualizar `whatsapp_id` automáticamente
   - Frontend llama a `/api/link-phone` con el número real
   - Ambos campos se actualizan cuando el frontend llama a `/api/link-phone`

## ✅ Implementación Actual (Recomendada)

El flujo actual funciona correctamente:

1. **Al conectar:** Se actualiza `whatsapp_id` automáticamente (si encuentra usuario vinculado)
2. **Frontend detecta conexión:** Llama a `/api/link-phone` automáticamente
3. **Backend actualiza:** Ambos campos (`whatsapp_id` y `whatsapp_number`) en `/api/link-phone`

**Ventajas:**
- Actualización automática de `whatsapp_id` al conectar
- Actualización de ambos campos cuando el frontend llama a `/api/link-phone`
- No requiere intervención manual del usuario
- Funciona tanto para usuarios nuevos como para reconexiones
